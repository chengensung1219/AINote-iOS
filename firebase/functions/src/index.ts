...
const assemblySecret = defineSecret('ASSEMBLYAI_API_KEY');
...
export const getAssemblyRealtimeToken = onRequest(
  {
    region: REGION,
    memory: '256MiB',
    timeoutSeconds: 15,
  ...
  async (req, res) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to setup streaming.';
      res.status(500).json({ error: message });
    }
  },
);

export const summarizeTranscript = onRequest(
  {
    region: REGION,
    memory: '512MiB',
    timeoutSeconds: 30,
...
export const reviewAnswer = onRequest(
  {
    region: REGION,
    memory: '512MiB',
    timeoutSeconds: 30,
  ...
  async (req, res) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to review.';
      res.status(500).json({ error: message });
    }
  },
);
...
async function reviewWithGemini(question: string, transcript: string): Promise<{ review: string, score: number }> {
  const client = genAI();
  if (!client) {
    throw new Error('Gemini client is not configured.');
  }
